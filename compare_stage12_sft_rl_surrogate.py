...
"""
compare_stage12_sft_rl_surrogate.py
Compare Stage12 SFT vs RL using a learned forward surrogate as metric.
"""
...
if __name__ == "__main__":
    ...
    model_sft, word_dict, _, meta = load_stage12(args.ckpt_sft, device)
    model_rl, word_dict_rl, _, _ = load_stage12(args.ckpt_rl, device)
    ...
    if word_dict_rl != word_dict:
        raise ValueError("RL vocab != SFT vocab. Check ckpt alignment.")

    pad_id = s_meta.get("pad_id", 1)
    max_len = args.max_len if args.max_len is not None else s_meta.get("max_len", meta.get("max_len", 128))

    m_sft = eval_model(model_sft, word_dict, spec_arr, surrogate, pad_id, args.n, max_len, args.greedy, args.n_candidates)
    m_rl = eval_model(model_rl, word_dict, spec_arr, surrogate, pad_id, args.n, max_len, args.greedy, args.n_candidates)

    tag = "greedy" if args.greedy else "sample"
    if args.n_candidates > 1:
        tag += f" best-of-{args.n_candidates}"

    print(f"Stage12 comparison (surrogate) [{tag}]")
