...
"""
compare_stage12_sft_rl.py
对比 Stage12-SFT 与 Stage12-RL 在同一 spec 上的 MSE/MAE/Corr。
"""
...
def eval_model(model, word_dict, index_dict, spec_arr, n, max_len, greedy=False, n_candidates=1):
    ...
    for _ in range(n):
        ...
        for _c in range(max(1, n_candidates)):
            ids = generate(model, spec, word_dict, index_dict, max_len, device, greedy=greedy)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt_sft", required=True)
    parser.add_argument("--ckpt_rl", required=True)
    ...
    model_sft, word_dict, index_dict, _ = load_ckpt(args.ckpt_sft, device)
    model_rl, _, _, _ = load_ckpt(args.ckpt_rl, device)

    m_sft = eval_model(model_sft, word_dict, index_dict, spec_arr, args.n, args.max_len, args.greedy, args.n_candidates)
    m_rl = eval_model(model_rl, word_dict, index_dict, spec_arr, args.n, args.max_len, args.greedy, args.n_candidates)

    tag = "greedy" if args.greedy else "sample"
    if args.n_candidates > 1:
        tag += f" best-of-{args.n_candidates}"

    print(f"Stage12 comparison [{tag}]")
    print("| Model | MSE | MAE | Corr |")
    print("|------|-----|-----|------|")
    print(f"| SFT | {m_sft[0]:.6f} | {m_sft[1]:.6f} | {m_sft[2]:.6f} |")
    print(f"| RL  | {m_rl[0]:.6f} | {m_rl[1]:.6f} | {m_rl[2]:.6f} |")
