...
"""
train_stage12_rl_surrogate.py
RL fine-tuning for Stage12 OptoGPT using a forward surrogate as reward.
...
from config_stage12_rl_surrogate import TrainConfig
...
def trim_after_eos(tokens, eos_id, pad_id):
    tokens = tokens.clone()
    for b in range(tokens.size(0)):
        eos_pos = (tokens[b] == eos_id).nonzero(as_tuple=False)
        if eos_pos.numel() > 0:
            first = int(eos_pos[0].item())
            if first + 1 < tokens.size(1):
                tokens[b, first + 1:] = pad_id
    return tokens
...
def compute_reward(tokens, spec_batch, word_dict, index_dict, cfg: TrainConfig, parser, validator, surrogate):
    rewards = []
    eos_id = word_dict.get("EOS", 3)
    pad_id = word_dict.get("PAD", 1)
    tokens = trim_after_eos(tokens, eos_id, pad_id)

    # surrogate prediction
    pred_spec = surrogate(tokens, pad_id)  # (B, spec_dim)
    mse = torch.mean((pred_spec - spec_batch) ** 2, dim=1)
    denom = (pred_spec.norm(dim=1) * spec_batch.norm(dim=1)) + 1e-8
    corr = torch.sum(pred_spec * spec_batch, dim=1) / denom
    ...
    for b in range(tokens.size(0)):
        ...
        if not ok:
            ...
            r = -cfg.reward_mse_weight * mse[b] + cfg.reward_corr_weight * corr[b]
...
def train():
    ...
    # override max_len from SFT ckpt
    if "max_len" in model_cfg:
        cfg.max_len = int(model_cfg["max_len"])

    # load surrogate
    s_ckpt = torch.load(cfg.surrogate_ckpt, map_location="cpu")
    surrogate, s_meta = build_surrogate_from_ckpt(s_ckpt, device)
    surrogate.eval()

    if s_meta.get("vocab_size", 0) != len(word_dict):
        raise ValueError("Surrogate vocab_size != Stage12 vocab_size. Check alignment.")
    ...
    for step in range(1, cfg.steps + 1):
        ...
        r_sample = compute_reward(sample_tokens, spec_batch, word_dict, index_dict, cfg, parser, validator, surrogate)

        base_tokens = generate_rollout(model, spec_batch, word_dict, token_strings, cfg, greedy=True)
        r_base = compute_reward(base_tokens, spec_batch, word_dict, index_dict, cfg, parser, validator, surrogate)
        ...
        if step % cfg.save_every == 0:
            ck = os.path.join(cfg.save_dir, f"stage12_rl_surrogate_step{step}.pt")
            payload = {
