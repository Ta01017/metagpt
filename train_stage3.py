...
@torch.no_grad()
def compute_reward(tokens, spectra, tk, parser, validator, cfg: TrainConfig):
    B = spectra.size(0)
    rewards = []
    for b in range(B):
        ids = tokens[b].tolist()
        has_eos = tk.eos_id in ids
        if has_eos:
        ...
        if not ok:
            r = torch.tensor(cfg.invalid_penalty, device=spectra.device)
        else:
            pred = spectrum_from_struct_det(struct_toks, spec_dim=spectra.size(1))
            pred_spec = torch.tensor(pred, device=spectra.device)
            mse = torch.mean((pred_spec - spectra[b]) ** 2)
            # corr
            denom = (torch.norm(pred_spec) * torch.norm(spectra[b])) + 1e-8
            corr = torch.dot(pred_spec, spectra[b]) / denom
            r = -cfg.reward_mse_weight * mse + cfg.reward_corr_weight * corr

        # length penalty (structure tokens only)
        if len(struct_toks) > 0:
            r = r - cfg.reward_len_penalty * len(struct_toks)

        # repeat penalty (max run of same token)
        max_run = 1
        run = 1
        last = None
        for t in toks:
        ...
        # extra penalty for common invalid reasons
        if not ok:
            if reason in ("rect_length_exceed", "rect_width_exceed"):
                r = r - 0.3
            if reason in ("missing_rect_dims", "missing_period", "bad_height"):
                r = r - 0.2
...
def train_stage3():
    ...
    prefix_len = int(meta.get("prefix_len", 32))
    ...
    for step in range(start_step, cfg.steps):
        ...
        r_sample = compute_reward(sample_tokens, spectra, tk, parser, validator, cfg)

        # greedy baseline
        base_tokens = generate_rollout(model, spectra, tk, cfg, greedy=True)
        r_base = compute_reward(base_tokens, spectra, tk, parser, validator, cfg)
